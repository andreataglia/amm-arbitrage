...
describe("AribtrageSaga", () => {
  ...
  before(async () => {
    ...
    uniswapV2Factory = new web3.eth.Contract(
      context.uniswapV2FactoryABI,
      context.uniswapV2FactoryAddress
    );
    liquidityPool = new web3.eth.Contract(
      context.uniswapV2PairABI,
      await uniswapV2Factory.methods
        .getPair(context.buidlTokenAddress, context.wethTokenAddress)
        .call()
    );

    ammDFO = await dfoManager.createDFO(
      "MyName",
      "MySymbol",
      10000000,
      100,
    ...
    var AMMPaths = await new Promise(function (ok) {
      var basePath = (path.resolve(__dirname, "..", "contracts") + "/")
        .split("\\")
        .join("/");
      glob(
        basePath + "amm-aggregator/models/**/!(I)*.sol",
        {},
        (err, files) => {
          ok(
            files.map((it) =>
    ...
    for (var AMMPath of AMMPaths) {
      AMMs[
        AMMPath.substring(AMMPath.lastIndexOf("/") + 1)
          .split("AMMV1")
          .join("")
      ] = await compile(AMMPath);
    }

    var uniswap = {
      contract: await new web3.eth.Contract(AMMs.UniswapV2.abi)
        .deploy({
          data: AMMs.UniswapV2.bin,
          arguments: [context.uniswapV2RouterAddress],
        })
    ...
    amms = {
      mooniswap: {
        contract: await new web3.eth.Contract(AMMs.Mooniswap.abi)
          .deploy({
            data: AMMs.Mooniswap.bin,
            arguments: [context.mooniswapFactoryAddress],
          })
          .send(blockchainConnection.getSendingOptions()),
      },
      uniswap,
      sushiSwap: {
        contract: await new web3.eth.Contract(AMMs.SushiSwap.abi)
          .deploy({
            data: AMMs.SushiSwap.bin,
            arguments: [context.sushiSwapRouterAddress],
          })
          .send(blockchainConnection.getSendingOptions()),
      },
      balancer: {
        contract: await new web3.eth.Contract(AMMs.Balancer.abi)
          .deploy({
            data: AMMs.Balancer.bin,
            arguments: [context.wethTokenAddress],
          })
          .send(blockchainConnection.getSendingOptions()),
      },
    };
    ...
    await Promise.all(
      tokens.map((it) => buyForETH(it, buyForETHAmount, uniswap.contract))
    );

    var ammsAddresses = Object.values(amms).map(
      (it) => it.contract.options.address
    );

    ammAggregator = await new web3.eth.Contract(AMMAggregator.abi)
      .deploy({
        data: AMMAggregator.bin,
        arguments: [ammDFO.doubleProxyAddress, ammsAddresses],
      })
      .send(blockchainConnection.getSendingOptions());
  });
    ...
    for (var address of addresses) {
      ...
      try {
        await addLiquidity(
          ...
          true,
          false,
          amm.doubleTokenLiquidityPoolAddress
        );
      } catch (e) {}
      try {
        amm.name.toLowerCase().indexOf("balancer") === -1 &&
  ...
  function getTokenAddresses(
    ...
    var tokenAddresses = [
      existing
        ? randomTokenAddress()
        : dfo
        ? dfo.votingTokenAddress
        : randomTokenAddress(),
    ];
    tokenAddresses.push(
      ethereum ? ethereumAddress : randomTokenAddress(tokenAddresses)
    );
  ...
  function getToken(tokenAddress, ethereum, ethereumAddress, dfo) {
    ...
    return tokenAddress === (dfo = dfo || {}).votingTokenAddress
      ? dfo.votingToken
      : tokens.filter((it) => it.options.address === tokenAddress)[0];
  }
  ...
  it("byLP", async () => {
    var data = await ammAggregator.methods
      .findByLiquidityPool(liquidityPool.options.address)
      .call();
    assert.strictEqual(
      amms.uniswap
        ? amms.uniswap.contract.options.address
  ...
  it("setAMMS", async () => {
    ...
    var code = fs
      .readFileSync(
        path.resolve(__dirname, "..", "resources/AMMSetAMMs.sol"),
        "UTF-8"
      )
      .format(
        ammAggregator.options.address,
        toDeleteCode.trim(),
        toAdd.length,
        toAddCode.trim()
      );
    var proposal = await dfoManager.createProposal(
      ammDFO,
      "",
      true,
      code,
  ...
  async function createLiquidityPoolAndAddLiquidityNoPool(
    amm,
    receiver,
    existing,
    ethereum,
    ...
    var realReceiver =
      receiver === utilities.voidEthereumAddress ? accounts[0] : receiver;
    ...
    if (existing) {
      ...
      data = await amm.contract.methods
        .byTokenAmount(liquidityPoolAddress, tokenAddresses[0], amounts[0])
        .call();
      expectedLiquidityPoolAmount = await liquidityPool.methods
        .balanceOf(realReceiver)
        .call();
      expectedLiquidityPoolAmount = web3.utils
        .toBN(expectedLiquidityPoolAmount)
        .add(web3.utils.toBN(data[0]))
        .toString();
      expectedLiquidityPoolAmount = utilities.fromDecimals(
    ...
    for (var i in tokenAddresses) {
      ...
      if (ethereum && tokenAddresses[i] === amm.ethereumAddress) {
        ...
        await token.methods
          .approve(amm.contract.options.address, amounts[i])
          .send(blockchainConnection.getSendingOptions());
      }
      expectedReceiverAmounts.push(
        token
          ? await token.methods.balanceOf(realReceiver).call()
          : await web3.eth.getBalance(realReceiver)
      );
      expectedSenderAmounts.push(
        token
          ? await token.methods.balanceOf(accounts[0]).call()
          : await web3.eth.getBalance(accounts[0])
      );
      expectedReceiverAmounts[i] = web3.utils
        .toBN(expectedReceiverAmounts[i])
        .add(web3.utils.toBN(amounts[i]))
        .toString();
      expectedSenderAmounts[i] = web3.utils
        .toBN(expectedSenderAmounts[i])
        .sub(web3.utils.toBN(amounts[i]))
        .toString();
    }

    var transaction = await amm.contract.methods
      .createLiquidityPoolAndAddLiquidity(
        tokenAddresses,
        amounts,
        ethereum,
        realReceiver
      )
      .send(blockchainConnection.getSendingOptions({ value: ethereumAmount }));
    var transactionFee = await blockchainConnection.calculateTransactionFee(
      transaction
    );
    ...
    if (!existing) {
      ...
      liquidityPoolAddress =
        ...
        web3.eth.abi.decodeParameter(
          ...
          transactionReceipt.logs.filter(
            (it) =>
              it.topics[0] ===
              web3.utils.sha3("NewLiquidityPoolAddress(address)")
          )[0].topics[1]
        );
      liquidityPool = new web3.eth.Contract(
        context.IERC20ABI,
      ...
      var liquidityPoolData = await amm.contract.methods
        .byLiquidityPool(liquidityPoolAddress)
        .call();
      var decimals = await liquidityPool.methods.decimals().call();
    ...
    for (var i in tokenAddresses) {
      ...
      var actualSenderBalance = token
        ? await token.methods.balanceOf(accounts[0]).call()
        : await web3.eth.getBalance(accounts[0]);
      assertstrictEqual(
        utilities.fromDecimals(actualSenderBalance, decimals),
        utilities.fromDecimals(expectedSenderAmounts[i], decimals)
      );
    }

    var liquidityPoolDecimals = await liquidityPool.methods.decimals().call();
    assertstrictEqual(
      utilities.fromDecimals(
        await liquidityPool.methods.balanceOf(realReceiver).call(),
  ...
  it("createLiquidityPoolAndAddLiquidity", async () => {
    for (amm of Object.values(await getAMMS())) {
      try {
        ...
        await createLiquidityPoolAndAddLiquidityNoPool(
          amm,
          accounts[1],
          false,
          true
        ...
        if (amm.maxTokensPerLiquidityPool == 0) {
          ...
          console.log(
            "receiver existing pool ethereum with more than 2 tokens"
          );
          await createLiquidityPoolAndAddLiquidityNoPool(
            amm,
            accounts[1],
          ...
          console.log(
            "no receiver existing pool ethereum with more than 2 tokens"
          );
          await createLiquidityPoolAndAddLiquidityNoPool(
            amm,
            undefined,
  ...
  it("UniswapBatch", async () => {
    ...
    if (!liquidityPoolAddress) {
      var data = await amm.contract.methods
        .byTokens([context.daiTokenAddress, amm.ethereumAddress])
        .call();
      liquidityPoolAddress = data[2];
    ...
    var liquidityPool = new web3.eth.Contract(
      context.IERC20ABI,
      liquidityPoolAddress
    );
    var before = utilities.fromDecimals(
      await liquidityPool.methods.balanceOf(accounts[0]).call(),
      18
    );
    ...
    for (var tokenAddress of tokenAddresses) {
      ...
      await token.methods
        .approve(
          amm.contract.options.address,
          utilities.toDecimals(
    ...
    var byLiquidityPoolAmountData = await amm.contract.methods
      .byLiquidityPoolAmount(liquidityPoolAddress, liquidityPoolAmount)
      .call();
    ...
    var value = web3.utils
      .toBN(ethereumAmount)
      .add(web3.utils.toBN(ethereumAmount))
      .toString();
    await amm.contract.methods
      .addLiquidityBatch(liquidityPoolsData)
      .send(blockchainConnection.getSendingOptions({ value }));
    await nothingInContracts(amm.contract.options.address);
    ...
    var after = utilities.fromDecimals(
      await liquidityPool.methods.balanceOf(accounts[0]).call(),
      18
    );
    ...
    await liquidityPool.methods
      .approve(
        ...
        utilities.toDecimals(
          0xfffffffffffffffffffffffffffffffffffffffffffff,
          await liquidityPool.methods.decimals().call()
        )
      )
      .send(blockchainConnection.getSendingOptions());
    await amm.contract.methods
      .removeLiquidityBatch(liquidityPoolsData)
      .send(blockchainConnection.getSendingOptions());
    await nothingInContracts(amm.contract.options.address);
    before = after;
    after = utilities.fromDecimals(
      await liquidityPool.methods.balanceOf(accounts[0]).call(),
      18
    );
    console.log(before, after);
  ...
  it("SushiSwap Batch", async () => {
    ...
    if (!liquidityPoolAddress) {
      var data = await amm.contract.methods
        .byTokens([context.usdcTokenAddress, amm.ethereumAddress])
        .call();
      liquidityPoolAddress = data[2];
      tokenAddresses = data[3];
    }
  ...
  it("Balancer Batch", async () => {
    ...
    var byTokenAmount = await amm.contract.methods
      .byTokenAmount(liquidityPoolAddress, amm.ethereumAddress, ethereumAmount)
      .call();
    ...
    byLiquidityPoolAmountData = await amm.contract.methods
      .byLiquidityPoolAmount(liquidityPoolAddress, liquidityPoolAmount)
      .call();
  ...
  async function swapLiquidity(
    ...
    var liquidityPoolAddress = !moreThanOne
      ? amm.doubleTokenLiquidityPoolAddress
      : amm.multipleTokenLiquidityPoolAddress;
    ...
    if (!liquidityPoolAddress) {
      ...
      while (
        (liquidityPoolAddress = (
          await amm.contract.methods
            .byTokens([
              (inputToken = enterInETH
                ? amm.ethereumAddress
                : randomTokenAddress(amm.ethereumAddress)),
              (otherToken = randomTokenAddress([
                inputToken,
                amm.ethereumAddress,
              ])),
            ])
          ...
          while (true) {
            ...
            if (otherToken !== inputToken && paths.indexOf(otherToken) === -1) {
              if (
                (liquidityPoolAddress = (
                  await amm.contract.methods
                    .byTokens([paths[position], otherToken])
                    .call()
                )[2]) !== utilities.voidEthereumAddress
              ) {
                break;
      ...
      if (exitInETH) {
        ...
        liquidityPoolAddresses[liquidityPoolAddresses.length - 1] = (
          await amm.contract.methods
            .byTokens([
              moreThanOne ? paths[paths.length - 2] : inputToken,
      ...
      var lpTokens = (
        await amm.contract.methods.byLiquidityPool(liquidityPoolAddress).call()
      )[2];
      ...
      if (amm.maxTokensPerLiquidityPool === 0 && moreThanOne) {
        ...
        var thirdToken = lpTokens.filter(
          (it) => it !== amm.ethereumAddress && it !== paths[0]
        )[0];
    ...
    if (
      enterInETH &&
      !exitInETH &&
      paths[paths.length - 1] === context.wethTokenAddress
    ) {
      return await swapLiquidity(
        amm,
        receiver,
    ...
    if (
      enterInETH &&
      exitInETH &&
      (paths.slice(0, paths.length - 1).indexOf(amm.ethereumAddress) !== -1 ||
        paths
          .slice(0, paths.length - 1)
          .indexOf(utilities.voidEthereumAddress) !== -1)
    ) {
      return await swapLiquidity(
        amm,
        receiver,
    ...
    var token =
      inputToken !== utilities.voidEthereumAddress &&
      tokens.filter((it) => it.options.address === inputToken)[0];
    var decimals = !token ? 18 : await token.methods.decimals().call();

    var lastTokenAddress = paths[paths.length - 1];
    var lastToken =
      lastTokenAddress !== utilities.voidEthereumAddress &&
      tokens.filter((it) => it.options.address === lastTokenAddress)[0];
    var lastDecimals = !lastToken
      ? 18
      : await lastToken.methods.decimals().call();

    var amountPlain = randomPlainAmount();
    var amount = utilities.toDecimals(amountPlain, decimals);

    var expectedSenderBalance = !enterInETH
      ? await token.methods.balanceOf(accounts[0]).call()
      : await web3.eth.getBalance(accounts[0]);
    if (accounts[0] !== realReceiver) {
      expectedSenderBalance = web3.utils
        .toBN(expectedSenderBalance)
    ...
    var outputAmount;
    try {
      outputAmount = await amm.contract.methods
        .getSwapOutput(inputToken, amount, liquidityPoolAddresses, paths)
        .call();
    } catch (e) {
      ...
      if (
        e.message.toLowerCase().indexOf("identical_addresses") !== -1 ||
        e.message.toLowerCase().indexOf("insufficient_input_amount") !== -1
      ) {
        return await swapLiquidity(
          amm,
          receiver,
    ...
    for (var toCheckToken of toCheck) {
      var address = toCheckToken.options
        ? toCheckToken.options.address
        : toCheckToken;
      totals[address] = totals[address] || "0";
      for (var entry of entries) {
        var amnt = entry[address];
        amnt &&
          (totals[address] = web3.utils
            .toBN(totals[address])
            .add(web3.utils.toBN(amnt))
            .toString());
      }
      if (
        address === (enterInETH ? utilities.voidEthereumAddress : inputToken)
      ) {
        totals[address] = web3.utils
          .toBN(totals[address])
          .sub(web3.utils.toBN(amount))
      ...
      if (
        address ===
        (exitInETH ? utilities.voidEthereumAddress : lastTokenAddress)
      ) {
        totals[address] = web3.utils
          .toBN(totals[address])
          .add(web3.utils.toBN(outputAmount))
          .toString();
      }
    }

    var expectedReceiverBalance = !exitInETH
      ? await lastToken.methods.balanceOf(realReceiver).call()
      : await web3.eth.getBalance(realReceiver);
    expectedReceiverBalance = web3.utils
      .toBN(expectedReceiverBalance)
      .add(web3.utils.toBN(outputAmount))
      .toString();
    ...
    !enterInETH &&
      (await token.methods
        .approve(amm.contract.options.address, amount)
        .send(blockchainConnection.getSendingOptions()));

    var transaction;
    try {
      transaction = await amm.contract.methods
        .swapLiquidity(swapData)
        .send(
          blockchainConnection.getSendingOptions({
            value: enterInETH ? amount : "0",
          })
        );
      var names = [];
      for (var i in paths) {
        ...
        var adr = paths[i];
        var tkn =
          adr === utilities.voidEthereumAddress
            ? adr
            : new web3.eth.Contract(context.IERC20ABI, adr);
        names.push(await tokenData(tkn, "symbol"));
      }
      console.log(
        `swapped ${amountPlain} ${await tokenData(
          enterInETH ? utilities.voidEthereumAddress : token,
          "symbol"
        )} -> ${
          ...
          exitInETH ? utilities.voidEthereumAddress : lastToken,
          "symbol"
        )}`
      );
    } catch (e) {
    ...
    expectedSenderBalance = utilities.fromDecimals(
      expectedSenderBalance,
      decimals
    );
    expectedReceiverBalance = utilities.fromDecimals(
      expectedReceiverBalance,
      lastDecimals
    );

    var actualSenderBalance = utilities.fromDecimals(
      !enterInETH
        ? await token.methods.balanceOf(accounts[0]).call()
        : await web3.eth.getBalance(accounts[0]),
      decimals
    ...
    assertstrictEqual(
      actualSenderBalance,
      expectedSenderBalance,
      accounts[0] === realReceiver &&
        (enterInETH ||
          exitInETH ||
          ethInTheMiddle ||
          inputToken === paths[paths.length - 1])
        ? amountPlain
        : 0
    );

    var actualReceiverBalance = utilities.fromDecimals(
      !exitInETH
        ? await lastToken.methods.balanceOf(realReceiver).call()
        : await web3.eth.getBalance(realReceiver),
      lastDecimals
    ...
    try {
      assert(
        utilities.formatNumber(actualReceiverBalance.split(",").join("")) >=
          utilities.formatNumber(expectedReceiverBalance.split(",").join("")) -
            (accounts[0] === realReceiver &&
            (enterInETH ||
              exitInETH ||
              ethInTheMiddle ||
    ...
    var toCheck = [utilities.voidEthereumAddress];
    toCheck.push(...tokens.map((it) => it));
    totals[utilities.voidEthereumAddress] = web3.utils
      .toBN(totals[utilities.voidEthereumAddress])
      .sub(web3.utils.toBN(transactionFee))
      .toString();
    for (var toCheckToken of toCheck) {
      var address = toCheckToken.options
        ? toCheckToken.options.address
        : toCheckToken;
      var dec = toCheckToken.methods
        ? await toCheckToken.methods.decimals().call()
        : 18;
      actuals[address] = actuals[address] || "0";
      for (var entry of entries) {
        var amnt = entry[address];
        amnt &&
          (actuals[address] = web3.utils
            .toBN(actuals[address])
            .add(web3.utils.toBN(amnt))
            .toString());
      }
      try {
        assertstrictEqual(
          utilities.fromDecimals(actuals[address], dec),
          utilities.fromDecimals(totals[address], dec),
          0.0009
        );
      } catch (e) {
        console.error(
          await tokenData(toCheckToken, "symbol"),
          utilities.fromDecimals(actuals[address], dec),
          utilities.fromDecimals(totals[address], dec),
          e.message
        );
